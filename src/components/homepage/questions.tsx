        ...
        <div className='flex flex-col max-w-sm font-sans sm:max-w-2xl lg:max-w-5xl xl:max-w-6xl px-3 sm:px-1 mx-auto'>
            <div className='flex flex-col bg-[#fff9eb] rounded-2xl md:rounded-[4rem] mt-5  pt-20 pb-24'>
                ...
                {questionsData.map((item, index) => (
                    <div key={index} className={`flex flex-col gap-4 mb-2 mx-4 md:mx-8 px-3 sm:px-5 md:px-6 py-2.5 md:py-4 ${expandedIndex === index ? 'rounded-3xl' : 'rounded-none'} border-b border-[#e5e7eb] ${expandedIndex === index ? 'bg-[#f7f1e4]' : 'bg-[#fff9eb]'}`}>
                        <div 
